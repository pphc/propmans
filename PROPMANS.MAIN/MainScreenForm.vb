...
Imports PROPMANS.RENTAL_MGMT_MOD
...
#Region "Local Procedures"
    ...
    '    For Each items As ToolStripMenuItem In MainMenuStrip.Items

    '        For Each menu As ToolStripMenuItem In items.DropDownItems

    '            If menu.Tag.ToString = "logoff" Then
    '                AddHandler menu.Click, AddressOf LogOffToolStripMenuItem_Click
    '            ElseIf menu.Tag.ToString <> "main" Then
    '                menu.Enabled = False
    '                AddHandler menu.Click, AddressOf GetToolStripMenuItem
    '            End If

    '            For Each submenu As ToolStripMenuItem In menu.DropDownItems
    '                If submenu.Tag.ToString <> "main" Then
    ...
    '                End If
    '                For Each subsubmenu As ToolStripMenuItem In submenu.DropDownItems
    '                    If subsubmenu.Tag.ToString <> "main" Then
    '                        subsubmenu.Enabled = False
    '                        If subsubmenu.Tag.ToString.Substring(0, 3) = "REP" Then
    '                            AddHandler subsubmenu.Click, AddressOf ShowReportDialogue
    '                        Else
    '                            AddHandler subsubmenu.Click, AddressOf GetToolStripMenuItem
    '                        End If
    '                    End If
    '                Next
    '            Next
    ...
    Private Sub SetSubMenus(mainmenu As ToolStripItemCollection)
        ...
        For Each submenu As ToolStripMenuItem In mainmenu
            ...
            SetSubMenus(submenu.DropDownItems)
        Next
    End Sub
    ...
    '    Dim childform As KryptonForm

    '    childform = GetChildInstance(mnuTag)
    '    If Not formQ.Contains(childform) Then
    '        formQ.Enqueue(childform)
    '    End If

    '    '' childform.FormBorderStyle = Windows.Forms.FormBorderStyle.Sizable
    '    ''childform.WindowState = FormWindowState.Maximized
    '    '' childform.StartPosition = FormStartPosition.CenterParent
    '    childform.Show()
    '    ''childform.BringToFront()

    '    If formQ.Count > 5 Then
    '        childform = formQ.Dequeue()
    '        childform.Close()
    '    End If
    ...
    Private Function GetChildInstance(ByVal menuTag As String) As KryptonForm
        ...
        Select Case menuTag
            ...
            Case "RMAgreement"
                newfrm = RMAgreementForm.Instance
            Case "Lease Contract"
                newfrm = LeaseContractForm.Instance
            Case "Remittances"
            ...
            Case "Billing Statements"
                ''newfrm = BillingPrintingForm.Instance
        ...
        For Each modulename As KeyValuePair(Of String, ModuleAccessType) In GlobalReference.CurrentUser.ModuleAccess
            Select Case modulename.Key
                ...
                Case "UT_NOTICE_DISCONNECTION"
                    DisconnectionNoticeToolStripMenuItem.Enabled = True
